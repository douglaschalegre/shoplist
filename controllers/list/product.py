...
def add_product_to_list(
    list_product_input: schemas.ListProductInput = Body(description='Product to add'),
    session: Session = Depends(get_session),
) -> models.List:
    """Add a product to a list"""
    return list_service.add_product_to_list(list_product_input=list_product_input, session=session)
...
def add_products_to_list(
    ...
    list_products_input: list[schemas.ListProductInput] = Body(description='Products to add'),
    session: Session = Depends(get_session),
) -> models.List:
    """Add products to a list"""
    return list_service.add_products_to_list(
        list_id=list_id, list_products_input=list_products_input, session=session
    )
...
def update_product_in_list(
    ...
    list_id: UUID = Path(description='List ID'),
    list_product_edit: schemas.ListProductEdit = Body(description='Product to update'),
    session: Session = Depends(get_session),
) -> models.ListProduct:
    """Update a list"""
    return list_service.update_product_in_list(
        list_id=list_id, product_id=product_id, list_product_edit=list_product_edit, session=session
    )
...
def delete_product_from_list(
    ...
    return list_service.delete_product_from_list(
        list_id=list_id, product_id=product_id, session=session
    )
