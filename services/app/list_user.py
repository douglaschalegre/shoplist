...
from repositories.app import (
    list_user as list_user_repository,
...
def get_list_users(
    ...
    '''Get all user_list'''
    return list_user_repository.get_list_users(
        list_id=str(list_id),
        session=session
    )
...
def add_users_to_list(
    list_user_input: schemas.ListUserInput,
    session: Session
) -> list[models.ListUser]:
    '''Add a user to a list'''
    # Check if list exists
    existing_list = list_repository.get_list_by_id(
        list_id=list_user_input.list_id,
        session=session
    )
    ...
    return list_user_repository.add_users_to_list(
